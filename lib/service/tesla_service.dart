...
class TeslaService {
	...
	static const String _googleGeocode = 'http://maps.googleapis.com/maps/api/geocode/json?latlng=';
	...
	StreamController _vehicleUpdateController = new StreamController.broadcast(sync:true);
	Stream get vehicleUpdate => _vehicleUpdateController.stream;
	StreamController _chargeUpdateController = new StreamController.broadcast(sync:true);
	Stream get chargeUpdate => _chargeUpdateController.stream;
	bool _chargeUpdated = false;
	StreamController _climateUpdateController = new StreamController.broadcast(sync:true);
	Stream get climateUpdate => _climateUpdateController.stream;
	bool _climateUpdated = false;
	StreamController _driveUpdateController = new StreamController.broadcast(sync:true);
	...
	Future postLogin(String username, String password) {
		String uri = 'user_session[email]=$username&user_session[password]=$password';
	...
	Future vehicles() {
		...
		return _http.get(_apiVehiclesUrl)
		 .then((HttpResponse response){
	  		_vehicleListcache = [];
	...
	Future vehicleState(num id, [bool singleRun = false]) {
		if ((_vehicleStateCache.containsKey(id) || _vehicleFirstRun != null) && !singleRun){
			return _vehicleFirstRun.then((_) => new Future.value(_vehicleStateCache[id]));
		}
		if (singleRun){
	...
	Future chargeState(num id, [bool singleRun = false]) {
		...
		if ((_chargeStateCache.containsKey(id) || _chargeFirstRun != null)  && !singleRun){ 
	...
	Future climateState(num id, [bool singleRun = false]) {
		//TODO: Issue if chargeFirstRun Errors?
		if ((_climateStateCache.containsKey(id) || _climateFirstRun != null)  && !singleRun){ 
			return _climateFirstRun.then((_) => new Future.value(_climateStateCache[id]));
		}
		if (singleRun){
	...
	Future driveState(num id, [bool singleRun = false]) {
		//TODO: Issue if chargeFirstRun Errors?
		if ((_driveStateCache.containsKey(id) || _driveFirstRun != null)  && !singleRun){ 
	...
	Future setTemps(num id, num C){
		String url = "$_apiVehiclesUrl/$id/$_apiSetTempsUrl?driver_temp=$C&passenger_temp=$C";
		return _simpleRequest(url);
	}
